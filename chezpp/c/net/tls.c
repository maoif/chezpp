...
ptr chezpp_net_tls_connect(uptr handle, int fd, const char *server_name) {
  chezpp_tls_context *ctx = ctx_from_handle(handle);
  SSL *ssl;
  ...
  if (ctx == NULL || ctx->ctx == NULL) return make_error_status_message("invalid TLS context");
  ssl = SSL_new(ctx->ctx);
  if (ssl == NULL) return openssl_error_status("failed to create TLS session");
...
ptr chezpp_net_tls_accept(uptr handle, int fd) {
  chezpp_tls_context *ctx = ctx_from_handle(handle);
  SSL *ssl;
  chezpp_tls_session *session;
  int rc;
...
ptr chezpp_net_tls_read(uptr handle, int size, int nonblocking) {
  chezpp_tls_session *session = session_from_handle(handle);
  ptr bv;
  size_t nread = 0;
  int rc;
...
ptr chezpp_net_tls_read_into(uptr handle, ptr bv, int start, int stop, int nonblocking) {
  chezpp_tls_session *session = session_from_handle(handle);
  size_t nread = 0;
  int rc;
...
ptr chezpp_net_tls_write(uptr handle, ptr bv, int start, int stop, int nonblocking) {
  chezpp_tls_session *session = session_from_handle(handle);
  size_t nwritten = 0;
  int rc;
  ...
  (void)nonblocking;
  if (session == NULL || session->ssl == NULL) return make_error_status_message("invalid TLS session");
  rc = SSL_write_ex(session->ssl, Sbytevector_data(bv) + start, (size_t)(stop - start), &nwritten);
  if (rc != 1) return ssl_result_status(session->ssl, rc, "TLS write failed");
